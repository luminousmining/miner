...
void common::tool::ToolMemoryTrace::show() const
{
    ...
    if (0 != currentAllocated)
    {
        ...
        {
            ss << "  Ptr: " << pair.first << " | Function: " << pair.second.functionName
               << " | Line: " << pair.second.line << " | Size: " << pair.second.size << " bytes" << std::endl;
        }
    }
