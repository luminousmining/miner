...
namespace common
{
    struct KernelGenerator
    {
      public:
        ...
        template<typename T>
        inline void addDefine(std::string const& name, T const& value)
        {
            static_assert(
                std::is_same<T, uint8_t>() || std::is_same<T, uint16_t>() || std::is_same<T, uint32_t>()
                || std::is_same<T, uint64_t>() || std::is_same<T, int8_t>() || std::is_same<T, int16_t>()
      ...
      protected:
        ...
        void writeKernelInFile(std::string const& source, std::string const& extension);
    };
}