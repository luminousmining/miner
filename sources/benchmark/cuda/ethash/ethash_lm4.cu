...
__device__ __forceinline__
void ethash_create_mix_hash_parallel(
    ...
    uint32_t const thread_id)
{
    ...
    // Process THREADS_PER_HASH nonces in groups of PARALLEL_HASH (2 passes of 4)
    #pragma unroll 1
    for (uint32_t lane_base{ 0u }; lane_base < THREADS_PER_HASH; lane_base += PARALLEL_HASH)
    {
        ...
        // Load initial mix states for PARALLEL_HASH different nonces
        #pragma unroll
        for (uint32_t p{ 0u }; p < PARALLEL_HASH; ++p)
        {
            uint32_t const lane_id{ lane_base + p };

            // Load all 4 seed quarters from this lane
            uint4 s0, s1, s2, s3;
            ...
            // Each thread selects its quarter based on mix_idx
            switch (mix_idx)
            {
            ...
            // init0[p] = seed[0].x from thread `lane_id` (already broadcast)
            init0[p] = s0.x;
        }

        // 64 DAG accesses per nonce — PARALLEL_HASH nonces simultaneously
        #pragma unroll 1
        for (uint32_t a{ 0u }; a < ACCESSES; a += 4u)
        {
            // Which thread to broadcast the computed DAG index from
            ...
            for (uint32_t b{ 0u }; b < 4u; ++b)
            {
                ...
                for (uint32_t p{ 0u }; p < PARALLEL_HASH; ++p)
                {
                    // Compute DAG index from this thread's mix state
                    uint32_t start_index{ fnv1((a + b) ^ init0[p], ((uint32_t*)&mix[p])[b]) };
                    start_index %= d_dag_number_item;
                    // Broadcast from thread t to ensure coalesced access
        ...
        // Reduce mix states and store results
        #pragma unroll
        for (uint32_t p{ 0u }; p < PARALLEL_HASH; ++p)
        {
    ...
    uint64_t const start_nonce)
{
    ...
    ethash_create_mix_hash_parallel(dag, state, seed, thread_id);
    ...
    uint32_t const threads)
{
    ...
    kernel_ethash_lm4<<<blocks, threads, 0, stream>>>
    (
        result,
        (uint4*)dag,
        nonce
