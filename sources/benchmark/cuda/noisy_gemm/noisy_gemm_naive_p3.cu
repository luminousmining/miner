///////////////////////////////////////////////////////////////////////////////
// Pearl NoisyGEMM — naive kernel p3
// Tile shape: tm=16, tn=16, noise rank r=128
// Higher noise rank means more accumulation steps per tile vs p1.
///////////////////////////////////////////////////////////////////////////////
#include <cuda.h>
...
constexpr uint32_t NOISE_RANK { 128u }; // r  — low-rank noise dimension


///////////////////////////////////////////////////////////////////////////////
__device__ __forceinline__
uint32_t rotl32_p3(uint32_t const x, uint32_t const n)
{
...
__global__
void kernel_noisy_gemm_naive_p3(
    ...
    uint32_t const               maxWinning)
{
    ...
    for (uint32_t step{ 0u }; step < numSteps; ++step)
    {
        ...
        if (rows == TILE_HEIGHT && cols == TILE_WIDTH)
        {
            ...
            accumState[step % 16u] = rotl32_p3(accumState[step % 16u], 13u)
...
bool pearl_naive_p3(
    ...
    uint32_t                            threads)
{
    kernel_noisy_gemm_naive_p3<<<blocks, threads, 0, stream>>>(
