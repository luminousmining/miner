...
__constant__ uint32_t const KECCAK_F800_ROUND_LM1[22] =
{
...
__global__
void kernel_keccak_f800_lm1()
{
    uint32_t flat_matrix[25];

    #pragma unroll
    for (uint32_t round = 0u; round < 22u; ++round)
    {
        keccak_f800_round_lm1(flat_matrix);
        flat_matrix[0] ^= KECCAK_F800_ROUND_LM1[round];
    }
}


__host__
bool keccak_f800_lm1(
    cudaStream_t stream,
    uint32_t const blocks,
    uint32_t const threads)
{
    kernel_keccak_f800_lm1<<<blocks, threads, 0, stream>>>();
