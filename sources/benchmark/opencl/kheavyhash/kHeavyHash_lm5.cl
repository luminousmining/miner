// kHeavyHash (Kaspa) OpenCL benchmark kernel -- lm5 (powHash midstate + heavy
// round-0 hoist).
//
// Correctness-only Layer 3: bit-identical to the CPU reference in
// sources/algo/kheavyhash/*.cpp. Same powHash midstate as lm4; additionally the
// heavy-step keccak folds its constant round-0 column reductions at compile time.
//
...
inline void kHeavyHashHoisted(uchar const* input, uchar* out)
{
    ...
    ulong const a0 = HEAVY_INITIAL_STATE[0] ^ load_le_u64(input + 0);
    ulong const a1 = HEAVY_INITIAL_STATE[1] ^ load_le_u64(input + 8);
    ulong const a2 = HEAVY_INITIAL_STATE[2] ^ load_le_u64(input + 16);
    ulong const a3 = HEAVY_INITIAL_STATE[3] ^ load_le_u64(input + 24);
    ...
    ulong const d1 = c0 ^ rol_u64(c2, 1);
    ulong const d2 = c1 ^ rol_u64(c3, 1);
    ulong const d3 = c2 ^ rol_u64(c4, 1); // rol_u64(c4,1) is compile-constant
    ulong const d4 = c3 ^ rol_u64(c0, 1);
