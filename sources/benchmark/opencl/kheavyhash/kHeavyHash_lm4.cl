// kHeavyHash (Kaspa) OpenCL benchmark kernel -- lm4 (powHash keccak midstate).
//
// Correctness-only Layer 3: bit-identical to the CPU reference in
// sources/algo/kheavyhash/*.cpp. The work-group computes the powHash state up to
// round-1 theta once (nonce excluded), then each work-item folds its nonce back
// in -- so the per-nonce work skips the shared midstate. This is the kernel that
// ships as the production `search`.
//
...
// kernel/crypto/keccak_f1600.cl (used by the heavy step below).
