// kHeavyHash (Kaspa) OpenCL benchmark kernel -- lm1 (matrix staged in LDS).
//
// Correctness-only Layer 3: bit-identical to the CPU reference in
// sources/algo/kheavyhash/*.cpp. Same pipeline as lm0; the matrix is staged in
// local memory once per work-group before the per-nonce matmul.
//
