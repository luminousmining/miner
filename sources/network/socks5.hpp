...
namespace socks5
{ // threw in the kitchen sink for error codes
        ...
        struct impl : error_category
        {
            ...
            std::string message(int ev) const override
            {
                switch (static_cast<result_code>(ev))
                {
                    case result_code::ok:
                        return "Success";
                    case result_code::invalid_version:
                        return "SOCKS5 invalid reply version";
                    case result_code::disallowed:
                        return "SOCKS5 disallowed";
                    case result_code::auth_method_rejected:
                        return "SOCKS5 no accepted authentication method";
                    case result_code::network_unreachable:
                        return "SOCKS5 network unreachable";
                    case result_code::host_unreachable:
                        return "SOCKS5 host unreachable";
                    case result_code::connection_refused:
                        return "SOCKS5 connection refused";
                    case result_code::ttl_expired:
                        return "SOCKS5 TTL expired";
                    case result_code::command_not_supported:
                        return "SOCKS5 command not supported";
                    case result_code::address_type_not_supported:
                        return "SOCKS5 address type not supported";
                    case result_code::failed:
                        return "SOCKS5 general unexpected failure";
                    default:
    ...
    {
        return error_code{ static_cast<std::underlying_type<result_code>::type>(se), get_result_category() };
    }
} // namespace socks5

template<>
struct boost::system::is_error_code_enum<socks5::result_code> : std::true_type
{
};
...
namespace socks5
{
    ...
    struct core_t
    {
        ...
        core_t(Endpoint const& target, Endpoint proxy) : _proxy(proxy), _request(target)
        {
        }

#pragma pack(push)
#pragma pack(1)
        enum class addr_type : uint8_t
        {
            IPv4 = 0x01,
            Domain = 0x03,
            IPv6 = 0x04
        ...
        struct wire_address
        {
            ...
            size_t payload_length() const
            {
                ...
                {
                    case addr_type::IPv4:
                        return sizeof(payload.ipv4);
                    case addr_type::IPv6:
                        return sizeof(payload.ipv6);
        ...
        struct response_t
        {
            ...
            auto tail_buffers()
            { // depends on head_buffers being correctly received!
                return std::array{
...
#pragma pack(pop)
        ...
        auto request_buffers() const
        {
            return _request.buffers();
        }
        auto response_head_buffers()
        {
            return _response.head_buffers();
        }
        auto response_tail_buffers()
        {
            return _response.tail_buffers();
        }
        ...
        error_code get_result(error_code ec = {}) const
        {
            ...
            {
                case proxy_reply::succeeded:
                    return result_code::ok;
                case proxy_reply::disallowed:
                    return result_code::disallowed;
                case proxy_reply::network_unreachable:
                    return result_code::network_unreachable;
                case proxy_reply::host_unreachable:
                    return result_code::host_unreachable;
                case proxy_reply::connection_refused:
                    return result_code::connection_refused;
                case proxy_reply::ttl_expired:
                    return result_code::ttl_expired;
                ...
                case proxy_reply::address_type_not_supported:
                    return result_code::address_type_not_supported;
                case proxy_reply::general_failure:
                    break;
            }
    ...
    struct async_proxy_connect_op
    {
      ...
      public:
        ...
        async_proxy_connect_op(Completion handler, Socket& s, EndpointOrQuery target, Endpoint proxy)
            : _core(target, proxy), _socket(s), _handler(std::move(handler))
        {
        }
    ...
    template<typename Socket, typename EndpointOrQuery, typename Endpoint = typename Socket::protocol_type::endpoint>
    error_code proxy_connect(Socket& s, EndpointOrQuery target, Endpoint proxy, error_code& ec)
    {
    ...
    template<
        typename Socket,
        typename Token,
        typename EndpointOrQuery,
        typename Endpoint = typename Socket::protocol_type::endpoint>
    auto async_proxy_connect(Socket& s, EndpointOrQuery target, Endpoint proxy, Token&& token)
    {
