...
//
// A faithful port of sources/algo/kheavyhash/opencl/kheavyhash.cl, itself
// KAT-verified bit-identical to the CPU reference (sources/algo/kheavyhash/).
// The constants are copied verbatim from that kernel (and the CPU reference).
// This header is nvcc-only; the correctness oracle is the host CPU reference,
// exercised by the known-answer suites in sources/algo/kheavyhash/tests/.
...
namespace kheavyhash_cuda
{
    ...
    __device__ __constant__
    uint64_t HEAVY_INITIAL_STATE[25] = {
    ...
    __device__ __constant__
    int ROTATIONS[24] = { 1, 3, 6, 10, 15, 21, 28, 36, 45, 55, 2, 14,
                          27, 41, 56, 8, 25, 43, 62, 18, 39, 61, 20, 44 };

    __device__ __constant__
    int PI_LANE[24] = { 10, 7, 11, 17, 18, 3, 5, 16, 8, 21, 24, 4,
                        15, 23, 19, 13, 12, 2, 20, 14, 22, 9, 6, 1 };


    __device__ __forceinline__
    uint64_t rotl64(uint64_t const x, int const k)
    {
        return (x << k) | (x >> (64 - k));
    }
    ...
    __device__ __forceinline__
    uint64_t loadLe64(uint8_t const* p)
    {
        uint64_t v = 0;
        for (int b = 0; b < 8; ++b)
        {
    ...
    __device__ __forceinline__
    void storeLe256(uint64_t const* state, uint8_t* out)
    {
        for (int w = 0; w < 4; ++w)
        {
            for (int b = 0; b < 8; ++b)
    ...
    __device__ __forceinline__
    void keccakF1600(uint64_t* const a)
    {
    ...
    __device__ __forceinline__
    void powHash(uint8_t const* prePowHash, uint64_t const timestamp, uint64_t const nonce, uint8_t* out)
    {
        uint64_t state[25];
        for (int i = 0; i < 25; ++i)
        {
    ...
    __device__ __forceinline__
    void kHeavyHash(uint8_t const* input, uint8_t* out)
    {
        uint64_t state[25];
        for (int i = 0; i < 25; ++i)
        {
    ...
    __device__ __forceinline__
    void heavyHash(uint16_t const* __restrict__ matrix, uint8_t const* __restrict__ hash1,
