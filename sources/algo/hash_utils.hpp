...
namespace algo
{
    ...
    template<typename T>
    inline std::string toHex(T const& rhs, bool const withZero = true)
    {
    ...
    template<typename T>
    inline bool isHashEmpty(T const& hash)
    {
    ...
    template<typename T>
    inline bool isEqual(T const& lhs, T const& rhs)
    {
        uint32_t const length{ sizeof(T) };
        for (uint32_t i{ 0u }; i < length; ++i)
        {
    ...
    template<typename T>
    inline T toHash(std::string const& str, algo::HASH_SHIFT shift = algo::HASH_SHIFT::NOTHING)
    {
        T           hash{};
        std::string hex{ str };
    ...
    inline HashTo toHash2(HashFrom const& src)
    {
        HashTo dst{};
    ...
    template<typename T>
    inline T decimalToHash(std::string const& input)
    {
        ////////////////////////////////////////////////////////////////////////
        T hash{};
        ...
        {
            auto const value{ (accs[i] < 10) ? castU8(accs[i] + '0') : castU8(accs[i] + 'A' - 0xA) };
    ...
    template<typename T>
    inline T toLittleEndian(T const& input)
    {
        T hash{};
        ...
        for (uint32_t i{ 0u }; i < lenght; ++i)
        {
            ...
            hash.ubytes[i >> 1] |= (((input.ubytes[index - 1] >= 'A') ? input.ubytes[index - 1] - 'A' + 0xA
    ...
    template<typename T>
    inline T toBigEndian(T const& input)
    {
        T hash{};
