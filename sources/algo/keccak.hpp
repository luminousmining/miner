...
namespace algo
{
    void keccak(uint64_t* const out, uint32_t bits, uint8_t const* data, uint32_t size);

    inline void keccakF1600(uint64_t* const state)
    {
    ...
    template<typename T>
    inline T keccak(T const& src)
    {
        T hash{};
        keccak(hash.word64, size_t(sizeof(T) * 8), src.ubytes, sizeof(T));
        return hash;
    }

    template<typename T, typename U>
    inline T keccak(U const& src)
    {
        T hash{};
        keccak(hash.word64, size_t(sizeof(T) * 8), src.ubytes, sizeof(U));
        return hash;
    }
